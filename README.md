...
- Fast / Strong / GUI 三层模型路由。模型只做推理，Skill 定义流程，Tool 才能触碰本地数据或界面。
...
复制 `.env.example` 为 `.env`，或在 shell 中设置同名环境变量。至少配置 Fast/Strong 模型使用的 provider API key；启用 Computer Use 时还需设置 `ADPILOT_GUI_BASE_URL`、`ADPILOT_GUI_API_KEY` 和 `ADPILOT_GUI_MODEL`。
...
DMG 会使用 `build/icon.icns` 和标准拖拽到 Applications 的安装窗口。`pnpm icon:mac` 可从 `build/icon.svg` 重建图标。未签名 DMG 适合本机测试；对外分发前仍需配置 Apple Developer ID 签名与 notarization。原生应用会先读取仓库当前目录的 `.env`，打包后则可读取 `~/Library/Application Support/AdPilot/.env`；也支持直接继承系统环境变量。
...
AdPilot is a local-first, approval-gated advertising optimization agent. Install with Node 22 and pnpm, run `pnpm install --frozen-lockfile && pnpm build`, create a workspace with `adpilot init`, then launch with `adpilot`. Pi is the sole agent runtime; UI-TARS is constrained to one visual grounding action at a time. No live mutation can bypass deterministic policy, independent risk review, user approval, and a one-time bound token.
