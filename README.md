...
原生 Computer Use 需要给实际启动 `adpilot` 的 Terminal/运行器授予“辅助功能”和“屏幕录制”权限。建议使用独立浏览器 Profile，只登录获授权的广告账户，并在客户 `accounts.yaml` 中限制应用与域名。未配置 GUI 模型时，产品仍可做分析，但不会执行视觉任务。
