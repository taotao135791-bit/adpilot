...
- 33 篇内置知识技能（27 篇广告 playbook + 6 篇 coding/harness/插件安全/发布流程）在构建期内嵌进产物（`scripts/build-knowledge-data.mjs`），决策轮注入压缩目录与触发匹配、规划轮按需注入全文；技能页统一展示内置、用户级与工作区来源，它们都只是参考知识，不授予任何工具、权限或执行权。
...
- 插件生态：可运行插件仍必须经过 Ed25519 签名、SHA-256 完整性校验、子进程 + VM 隔离、能力 broker 与权限 diff 复核；插件页另有来自 GitHub、Google Drive、Figma 官方来源的候选集成卡，但明确不可安装。候选只有在固定版本、完成许可证/权限/隐私审查、隔离测试并由仓库外发布密钥签名后才能晋升精选目录。
