...
export async function createServer(system: AdPilotSystem, options: {
  ...
  if (options.desktopNative) {
    ...
    app.addHook("onRequest", async (request, reply) => {
      ...
      // web API. Bind every API request plus the event stream (which carries
      // client/task/plugin state) to the random token held by this exact
      // Electron instance so another local process cannot reuse it.
      if (!path.startsWith("/api/") && path !== "/events") return;
