...
AdPilot is a router for paid-media work. Keep this file lean: route the task,
...
1. Read optimizer profile files in the current working directory if present:
   `ADPILOT_OPTIMIZER.md`, `optimizer-profile.md`, or
   `.adpilot-optimizer.md`.
...
This router may run from a manual AdPilot install or from a plugin/source tree.

- Manual install: router at `~/.adpilot/skills/ads/SKILL.md`, sub-skills at
  `~/.adpilot/skills/ads-*/SKILL.md`.
...
- When the route table says load `ads-google`, read that sub-skill's `SKILL.md`
  from the first existing path: `~/.adpilot/skills/ads-google/SKILL.md`,
...
Use the first existing template directory: `~/.adpilot/skills/ads-ops/assets/`,
