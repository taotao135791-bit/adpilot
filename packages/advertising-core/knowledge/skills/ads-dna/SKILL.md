...
```bash
python ~/.adpilot/skills/ads/scripts/capture_screenshot.py [url]
```
Saves: `./brand-screenshots/{domain}_homepage.png`

2. **Product or services page**:
```bash
python ~/.adpilot/skills/ads/scripts/capture_screenshot.py [url]/products
```
Saves: `./brand-screenshots/{domain}_product.png`

3. **About page** (brand personality):
```bash
python ~/.adpilot/skills/ads/scripts/capture_screenshot.py [url]/about
```
...
Read `~/.adpilot/skills/ads/references/brand-dna-template.md` for the exact schema.
...
(where the user is running AdPilot CLI).
