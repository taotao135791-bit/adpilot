...
def _load_replay(case_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    if contract == "six-stage":
        ...
        documents["decision-at-the-time.yaml"] = {
            **human_decision,
            "adpilot_ads": system_recommendation.get("adpilot_ads"),
        }
    return documents
...
def evaluate_replay(
    ...
    *,
    ...
    decision_data = recorded_decision.get("adpilot_ads")
    ...
    if recorded_feasibility not in FEASIBILITY_STATES:
        raise ContractError("adpilot_ads.feasibility is invalid")
    recorded_confidence = decision_data.get("confidence")
    if recorded_confidence not in {"low", "medium", "high"}:
        raise ContractError("adpilot_ads.confidence is invalid")
