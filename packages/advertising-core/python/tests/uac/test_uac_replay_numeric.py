...
from adpilot_ads.uac.replay import (  # noqa: E402
    REPLAY_FILES,
    evaluate_replay,
    render_replay,
    replay_path,
)
from adpilot_ads.uac.policy_loader import load_policy_set  # noqa: E402
from adpilot_ads.uac.types import ContractError  # noqa: E402

replay_module = importlib.import_module("adpilot_ads.uac.replay")
...
def make_numeric_case(repo_root: Path, tmp_path: Path) -> Callable[[str], Path]:
    ...
    def factory(name: str) -> Path:
        ...
        system = documents["system-recommendation.yaml"]["adpilot_ads"]
...
def test_mature_operational_action_can_score_without_claiming_an_experiment(
    ...
    documents["system-recommendation.yaml"]["adpilot_ads"]["created_experiment"] = False
    documents["evaluation.yaml"].update(
        {
            "experiment_completed": False,
            "conclusive": False,
...
def test_legacy_replay_can_add_numeric_labels_without_claiming_unknown_acceptance(
    ...
    documents["decision-at-the-time.yaml"] = {
        ...
        "adpilot_ads": system["adpilot_ads"],
    }
    for filename in REPLAY_FILES:
