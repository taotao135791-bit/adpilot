const SENSITIVE_KEY = /(text|password|token)/i;
const MAX_DEPTH = 6;
 ...
 * sensitive keys (text/password/token — free-form content and credentials
 * must never land in an audit record) and caps depth/size so a chatty
 * parameter cannot bloat the audit log.
 */
